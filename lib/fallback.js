...
module.exports = (previousError, request, response) => {
  ...
  const workbook = excelbuilder.createWorkbook(request.reporter.options.tempAutoCleanupDirectory, generationId + '.xlsx')
  ...
  return Promise.promisify(workbook.save).call(workbook).then(() => {
    response.stream = fs.createReadStream(path.join(request.reporter.options.tempAutoCleanupDirectory, generationId + '.xlsx'))
