...
module.exports = async (previousError, reporter, request, response) => {
  ...
  return Promise.promisify(workbook.save).call(workbook).then(() => {
    response.stream = fs.createReadStream(path.join(reporter.options.tempAutoCleanupDirectory, generationId + '.xlsx'))
    return responseXlsx(reporter, request, response)
  }).catch(function (e) {
