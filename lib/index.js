...
module.exports = (reporter, definition) => {
  ...
  reporter.documentStore.registerComplexType('XlsxTemplateRefType', {
    shortid: { type: 'Edm.String', schema: { type: 'null' } },
    templateAssetShortid: { type: 'Edm.String', schema: { type: 'null' } }
  })
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'xlsxTemplates', async (req) => {
    ...
    const findTemplate = async () => {
      if (
        !req.template.xlsxTemplate ||
        (!req.template.xlsxTemplate.shortid && !req.template.xlsxTemplate.templateAssetShortid && !req.template.xlsxTemplate.content && !req.template.xlsxTemplate.templateAsset)
      ) {
      ...
      if (req.template.xlsxTemplate.templateAsset && req.template.xlsxTemplate.templateAsset.content) {
        return parse(Buffer.from(req.template.xlsxTemplate.templateAsset.content, req.template.xlsxTemplate.templateAsset.encoding || 'utf8'))
      ...
      if (req.template.xlsxTemplate.templateAssetShortid) {
        docs = await reporter.documentStore.collection('assets').find({ shortid: req.template.xlsxTemplate.templateAssetShortid }, req)
      } else {
        docs = await reporter.documentStore.collection('xlsxTemplates').find({ shortid: req.template.xlsxTemplate.shortid }, req)
      }
